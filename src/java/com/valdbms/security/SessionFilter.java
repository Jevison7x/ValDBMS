        ...
        try
        {
            ...
            if(session != null && session.getAttribute("user") != null)
            {
                ...
                if(user != null && user.getEmail() != null)
                    if(requestIsAjax(wrappedRequest))
                        chain.doFilter(wrappedRequest, wrappedResponse);
                    else if(wrappedRequest.getRequestURI().contains("index"))
                        chain.doFilter(wrappedRequest, wrappedResponse);
                    else
                    {
                        RequestDispatcher dispatcher = wrappedRequest.getRequestDispatcher("index");
                        dispatcher.forward(request, response);
                    }
                else if(requestIsAjax(wrappedRequest))
                    wrappedResponse.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                else
                    wrappedResponse.sendRedirect("../login-page?login=3");
            }
            else if(requestIsAjax(wrappedRequest))
                wrappedResponse.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            else
